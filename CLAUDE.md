...
> Открытые задачи/идеи (реализовать / проресёрчить / отложено) —
> `docs/backlog.md`; баги конкретного скрипта — в его плейбуке, не там.
...
- **Инвариант версии**: VERSION-файл == верх CHANGELOG == последний тег. Сверять
  ...
  тег не двигали).
- Коммиты: Conventional Commits, тело на русском. **Без футеров-атрибуций**
  (`Co-Authored-By`, «Generated with …») — ни в коммитах, ни в PR-боди:
  автор репозитория один. Оформлено настройкой `attribution` в
  `.claude/settings.json`, но правило действует и при её отсутствии.
...
- **«Нет достижений» — терминально только от SAM**: пометить игру «без
  достижений» (`without.txt`, `mark_no_achievements`) вправе ТОЛЬКО SAM. Не
  хоронить игру по внешним догадкам — если статус неясен, она доезжает до SAM.
