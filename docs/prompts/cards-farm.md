...
3. `setup_logging(name='farm_cards', category='cards/farm')` (farm.py:190-192)
...
CLI-флаги (argparse в `_build_parser`, farm.py:265) — существует ТОЛЬКО один:
- `--reset` (store_true) — сбросить прогресс (удалить `cards/done.txt`) и начать заново
НЕТ флагов `--retry-errors` / `--no-*` / `-v/--verbose` / интервала / concurrency.
...
- `app/cards/card_cache.py` — прогресс фарма (txt-кэш): `CARD_DONE_FILE = CARDS_DIR/'done.txt'`; `mark_card_done(game_id)` append через `_append_id`, `clear_card_progress()` unlink done.txt. Семантика done.txt: игры с 0 оставшихся дропов. (Примечание: мёртвый card_store + CARD_HAS/NO_CARDS_FILE удалены в чистке 2026-07-02; осиротевший `load_card_done_ids` удалён после снятия GUI.)
- `app/cards/__init__.py` — фасад, реэкспорт: `clear_card_progress`, `mark_card_done` (из card_cache); `check_cards_remaining`, `fetch_games_with_card_drops` (из card_checker).
