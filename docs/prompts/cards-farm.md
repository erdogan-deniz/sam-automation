...
main() flow:
1. `print()` пустая строка
2. `setup_logging(name='farm_cards', category='cards/farm')`
...
- `_open_next(queue, active, cfg, game_names)` — запускает пока есть место и логирует APP NAME / APP PID / APP CARDS + SEPARATOR
- `_farm_loop(games_with_drops, cfg, cookies, steam_id)` — главный цикл
- `main()`

CLI-флаги: НЕТ (argparse убран в v1.9.x). `--reset` удалён — `cards/done.txt`
write-only, очередь строится из живого скрейпа badges, повторно карты не
выбиваются, так что чистить прогресс незачем. Нет также `--retry-errors` /
`-v/--verbose` / интервала / concurrency.

# КЛЮЧЕВЫЕ ФАЙЛЫ
- `scripts/cards/farm.py` — точка входа фарма; `main`, `_farm_loop`, `_open_next`, `_kill_game`. Без CLI-флагов (argparse снят в v1.9.x). Локальные имена игр берёт через `load_game_names()` из `app.cache`.
...
- `app/cards/card_cache.py` — прогресс фарма (txt-кэш): `CARD_DONE_FILE = CARDS_DIR/'done.txt'`; `mark_card_done(game_id)` append через `_append_id`. Семантика done.txt: игры с 0 оставшихся дропов, write-only (для пропуска НЕ читается). (Примечание: мёртвый card_store + CARD_HAS/NO_CARDS_FILE удалены в чистке 2026-07-02; осиротевший `load_card_done_ids` удалён после снятия GUI; `clear_card_progress` удалён вместе с флагом `--reset` в v1.9.x.)
- `app/cards/__init__.py` — фасад, реэкспорт: `mark_card_done` (из card_cache); `check_cards_remaining`, `fetch_games_with_card_drops` (из card_checker).
...
# НАЙДЕНО АУДИТОМ 2026-08-10 (№1 ИСПРАВЛЕН, №2 ОТКРЫТ — сверено 2026-09-09)
> Полный проектный аудит (46 агентов). Прежний аудит (82 агента, 2026-07-02)
> устарел — эти 2 находки НОВЫЕ, найдены свежим прочтением кода. Полные
> evidence — память `project_full_audit_2026_08_10`.

1. ~~**[Medium]** `app/cards/card_checker.py:180` — `_AuthError` маскируется
   под генерик-ошибку. `_fetch_page_with_retry` re-raise'ит `_AuthError`
   (401/403, протухшие куки) БЕЗ ретрая — корректно. Но
   `fetch_games_with_card_drops` ловит её только как generic `RuntimeError`,
   логирует вводящее в заблуждение «Стр. N не читается после M попыток»
   (ретрая не было), и после `_MAX_CONSEC_PAGE_FAILURES=3` подряд страниц
   абортит с частичным/пустым результатом. В отличие от `_farm_loop`
   (main.py:163-181), которая на `-1` от `check_cards_remaining` УЖЕ умеет
   рефрешить куки нон-интерактивно — сюда тот же паттерн не пробрасывается.
   Итог: `main()` видит `games_with_drops == []` и печатает ЛОЖНОЕ «всё уже
   получено» вместо честного «сессия протухла», молча скипая весь фарм.~~
   ИСПРАВЛЕНО: `_AuthError` стала публичным `AuthError`, `scripts/cards/farm.py`
   (строки 338, 354) ловит его отдельно, рефрешит куки нон-интерактивно и
   повторяет один раз, при повторном провале — честный exit.
2. **[Low, открыто] Вводящий в заблуждение комментарий** (строка 49-51 —
   теперь `scripts/cards/farm.py:50-51`, `_MAX_CHECK_FAILURES`):
   `_MAX_CHECK_FAILURES = 5  # после N неудачных проверок подряд считаем
   дропы закончившимися` — реальный код (213-225) делает ПРОТИВОПОЛОЖНОЕ:
   явно логирует «бросаю (НЕ помечаю done)» и кладёт appid в `unverified`
   (честный отчёт, не терминальный done). Риск: будущая правка «упростит»
   по комментарию в реальный done-марк. Переформулировать комментарий.

Общий баг в `app/run_lock.py` (`_proc_create_time`/`_is_live_owner`
трактует `psutil.AccessDenied` как «владелец мёртв») затрагивает и этот
скрипт, и achievements/farm.py, и boost.py — файл общий, не дублируй фикс
(проверь `git log`/`git status` перед правкой). Скрипт также использует
`get_web_cookies` (через `app.steam`) — 3 находки по самой fallback-цепочке
(playwright.py/web_refresh.py) см. в `docs/prompts/core-auth-cookies.md`.
