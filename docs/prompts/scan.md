...
farm/boost). Источники (порядок = приоритет первого вхождения):
...
скан НЕ прерывается. CLI: единственный флаг `--allow-shrink` (обход
floor-guard усадки all.txt, см. РИСКИ); иначе `python scripts/scan.py`,
конфиг из config.yaml.

# ТОЧКА ВХОДА: scripts/scan.py
- main(): setup_logging(category="achievements/scan") → load_config → validate →
  resolve_steam_id(api_key, steam_id) (vanity-имя / URL профиля → ID64) →
  ...
  floor-guard: усадка >50% vs prev_ids без `--allow-shrink` → ERROR + exit(1),
  all.txt не трогается → all.txt АТОМАРНО (`_atomic_write_text`) перезаписывается
  как "\n".join(sorted(combined)).
- _read_vdf_ids(steam_path, steam_id): read_library_app_ids; нет steam_path →
  warning + [].
- _read_api_ids(api_key, steam_id): fetch_owned_games → save_game_names(names)
  (best-effort в отдельном try — сбой записи имён НЕ роняет список) → список
  appid (api_key гарантирован validate — Phase 1 падает без него).
...
- app/steam/steam_local.py — read_library_app_ids: парсит
  ...
  (quote-aware токенайзер + стековый walker; App ID = numeric-ключи depth=1
  внутри apps). find_steam_path/steamid64_to_id3 — реэкспорт.
...
- app/steam/steam_id.py — resolve_steam_id: ID64 as-is / vanity-имя / URL
  профиля → ID64 (через ResolveVanityURL, требует api_key).
...
- Тесты (tests/unit/): test_scan (оркестрация main(): слияние 3 источников,
  floor-guard усадки, атомарная запись, resolve), test_steam_local (парс VDF),
  test_steam_id (resolve), test_steam_api, test_steam_cm (классификация логина),
  test_steam_registry, test_cache_game_names, test_id_file, test_game_list,
  test_network_hardening.

# КОНФИГ (config.yaml)
- steam_id — обязателен (17-значный / vanity-имя / URL); vanity и URL
  резолвятся в ID64 через resolve_steam_id (нужен api_key).
- steam_api_key — обязателен: validate требует (Phase 1 падает без него),
  нужен и для resolve_steam_id, и для источника API (+ names.json).
...
- all.txt ПЕРЕЗАПИСЫВАЕТСЯ (АТОМАРНО) объединением того, что вернули источники
  ЭТОТ прогон (prev_ids на диске НЕ мержится — только для отчёта «новых» и
  сравнения усадки). Защита: floor-guard блокирует усадку >50% vs прежнего
  all.txt (ERROR + exit(1), файл не трогается; обход `--allow-shrink`), но
  усадка ≤50% проходит молча (напр. транзиентный отказ CM теряет license-only
  игры). Инвариант «потерянная игра переоткроется следующим скрейпом» держится
  на повторных прогонах — учитывай при диагностике «пропавших» игр.
