...
- main(): args → setup_logging → load_config → resolve_steam_id (ДО validate:
  validate шлёт steam_id в GetPlayerSummaries, которому нужен числовой ID64 —
  сырой vanity/URL иначе даёт ложное «API key invalid») → validate →
  check_steam_running → ensure_sam → (не --list: acquire_run_lock ДО
...
- _fetch_targets(cfg, steam_id) → (games, blind): вселенная — all.txt (вся
  библиотека); played = {appid: playtime_forever} из fetch_owned_games (Steam
  API); hard-skip = exclude_ids ∪ playtime/skip.txt; done = playtime/done.txt
  (отдельно) → _select_targets. blind = (played пуст) — прогон «слепой»,
  вызывающий НЕ персистит done (проверить нечем).
- _select_targets(...): hard-skip пропускает любые игры; known-игры гейтятся по
  played/target (done.txt их НЕ глушит — самоисцеление после транзиентно-пустого
  owned); done.txt глушит игру ТОЛЬКО пока она unknown (resume). known=True →
  готовность по API (в done.txt НЕ пишем); known=False (API не отдаёт playtime:
  free/демо/лицензии) → идлим вслепую и в НЕслепом прогоне пишем done.
- _boost_loop(games, cfg, persist_done): по батчам concurrent_games:
  launch_games_staggered(stagger) → idle_and_split_survivors (поллинг весь idle
  монотонными часами, on_failed → skip ТОЛЬКО для unknown; known-провал НЕ
  хоронится — перепроверит API) → known НЕ в done, unknown → mark_playtime_done
  (только если persist_done) → пауза _PAUSE_AFTER_KILL=5с между батчами.
  try/except/finally: _teardown(active) на ЛЮБОМ выходе (норма/Ctrl+C/ошибка)
  НИКОГДА не бросает — добивает active + kill_all_sam_games (не осиротить SAM),
  повторный Ctrl+C → bounded-retry — поэтому _report_result всегда отрабатывает.
  done пишется только в норме. Итог через _report_result: ✅ только при полном
  прогоне без провалов, иначе ⚠️ (прервано/ошибка/с оговорками).
...
- app/steam/steam_api.py — fetch_owned_games (playtime_forever);
  app/steam/steam_id.py — resolve_steam_id (vanity/URL → ID64).
- app/sam/launcher.py — launch_games_staggered, idle_and_split_survivors
  (поллинг монотонными часами, on_failed→skip, гарды idle≤0/poll≤0/neg-sleep),
  kill_process, kill_all_sam_games (win32-свип сирот).
- app/sam/ — check_steam_running, ensure_sam.
- app/run_lock.py — acquire_run_lock/release_run_lock (data/.sam_run.lock).
- app/config.py — конфиг (_num ловит ValueError/TypeError/OverflowError на
  нечисловом/`.inf`); app/notify.py — toast + send_telegram; app/validator.py —
  validate (playtime idle 1..86400/target≥1/stagger finite 0..3600, isfinite).
...
- Пустой owned-games API при непустой all.txt (приватные Game details ИЛИ
  транзиентно-битый ответ) → warning + blind=True: ВСЕ игры считаются unknown и
  набиваются вслепую, но done НЕ персистится (иначе один сбой API навсегда
  пометил бы done всю библиотеку). Не абортим (all-Family-Share легитимен).
...
# НАЙДЕНО АУДИТОМ 2026-08-10
> Полный проектный аудит (46 агентов) сделал sanity-реверификацию этого
> скрипта (он уже ЗАКРЫТ через v1.14.1) — регрессий в самом boost.py не
> нашлось, статус «доведён» остаётся в силе.

Единственная новая находка касается ОБЩЕГО файла, не логики этого скрипта:
**[High] `app/run_lock.py:33` — `_proc_create_time`/`_is_live_owner`
трактуют `psutil.AccessDenied` как «владелец мёртв».** Живой владелец лока,
недоступный для опроса (напр. elevation-mismatch — один скрипт запущен
админом, другой нет), тихо считается мёртвым → второй SAM-спавнящий скрипт
может украсть лок и запуститься параллельно, прямое нарушение run-lock
инварианта. Тот же файл использует achievements/farm.py и cards/farm.py —
не дублируй фикс, если другая сессия уже взялась (проверь `git log
--oneline -5 -- app/run_lock.py` и `git status` перед правкой). Полный
