...
- main(): args → setup_logging → load_config → validate → _prepare_progress
  (--reset → clear_playtime_progress, чистит ТОЛЬКО playtime/done.txt) →
  check_steam_running → ensure_sam → resolve_steam_id → _fetch_targets →
  (если --list: печать и выход) → acquire_run_lock("playtime/boost") →
  atexit release → _boost_loop.
...
- _boost_loop(games, cfg): по батчам concurrent_games:
  ...
  on_failed → playtime/skip.txt) → known НЕ в done, unknown → mark_playtime_done
  → пауза _PAUSE_AFTER_KILL=5с между батчами.
...
- app/cache.py — пути и хелперы: PLAYTIME_DONE_FILE (data/games/ids/playtime/
  ...
  load/mark_playtime_skip, clear_playtime_progress.
...
- app/config.py — конфиг; app/notify.py — toast; app/validator.py — validate.
...
- done.txt — ТОЛЬКО для unknown-игр (API не видит playtime). known-игры
  ...
  _fetch_targets, а не флагом. --reset чистит done.txt, НЕ skip.txt.
- Детект провалившихся запусков — idle_and_split_survivors (поллинг весь idle):
  успех = процесс пережил весь idle без окна ошибки; провал → on_failed→skip.txt.
  (Старый гоночный drop_failed_launches заменён.)
