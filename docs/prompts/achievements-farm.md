...
_process_one_game по каждой игре → kill_process(proc) в finally → toast+telegram.
...
- _process_one_game, _apply_resume_filter, main() — БЕЗ покрытия; фейка session
  (add_and_open_game/close_game/launch_picker) нет.
- Реальный pywinauto-путь против живого SAM не тестируется (только логика на фейках).
- Итоговый тост/telegram и обработка прерываний в main не проверяются.
