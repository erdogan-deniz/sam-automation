...
адверсариальная сверка по коду).

ПРИМЕЧАНИЕ (2026-07-02): часть примеров-зацепок в таксономии УЖЕ устранена и
оставлена лишь как иллюстрация категорий — проверяй актуальность на месте:
  • дрейф доков (README «3.10+»→3.12, вкладка playtime, store_empty.txt,
    has_cards/no_cards, дерево playtime/, docstrings scan.py/runner.py) —
    исправлен (commit 409fb74 «docs: устранить дрейф документации»);
  • мёртвый card_store.py + транзитивные символы — удалён (feature/remove-dead-card-store).
Открытыми на момент сохранения остаются структурные пункты C (толстые scripts/*)
и E (непокрытый тестами app/cookies/*).
-->
...
- VERSION=1.5.0 сейчас консистентен (CHANGELOG top [1.5.0], тег/коммит a27bcc5, PR #9). Перед релизом пересверить.
