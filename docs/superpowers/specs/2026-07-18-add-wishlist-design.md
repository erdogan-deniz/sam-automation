...
и честный отчёт — обязательны (инвариант проекта). Add измеренно быстрый (~2/сек,
см. ниже), но каталог — сотни тысяч позиций, а устойчивый предел за один прогон
не измерен, поэтому фича остаётся **резюмируемой и потенциально многозапусковой**:
прогресс сохраняется, прогон можно прервать/продолжить в любой момент.
...
### Кап и rate-limit — ИЗМЕРЕНО вживую 2026-07-18 (отменяет догадку «~50/час»)
...
- **Скорость add — НАМНОГО выше догадки.** Ramp-замер: **40 добавлений подряд
  без пауз за 18.5с = ~2.2 add/сек ≈ 7700/час, ноль троттла** (`x-eresult=1` на
  всех, ни одного 429/84). Догадка «~50/час» была ошибочно перенесена из
  free-license adds (тяжёлый CM-грант пакета) — вишлист-add это лёгкая вставка
  строки, что подтверждает и опыт владельца (вишлист 24k собран кликами без
  блоков). **Фиксированный медленный интервал (72с) отвергнут.**
- **Реальный governor — адаптивный backoff по сигналу Steam, а не выдуманное
  число.** Идём быстро (небольшая вежливая пауза), а если Steam когда-либо
  ответит `429`/`x-eresult=84` — уважительно отступаем. При долбёжке 429 → IP
  soft-ban ~6ч (документировано для др. эндпоинтов Steam) → **backoff-then-stop,
  НЕ бесконечный ретрай** (отличие от free_games — см. ниже).
- **Не измерено:** истинный *устойчивый* потолок за тысячи adds в одном прогоне
  (burst 40 чист; дальше не долбили, чтобы не рисковать soft-ban). Это НЕ
  проблема дизайна — адаптивный backoff корректно подстроится под любой реальный
  предел без хардкода числа.
...
3. По одному appid (batch-эндпоинта нет):
   `POST AddToWishlist?access_token=… appid=…`. Между add — **небольшая вежливая
   пауза (дефолт ~1с)**, НЕ фиксированный медленный интервал: замер показал ~2.2
   add/сек без троттла, так что 1с — уже вежливо-быстро (~3600/час) и оставляет
   запас. `--interval` override (в т.ч. `0` для макс. скорости). Настоящий
   ограничитель — адаптивный backoff ниже, не эта пауза.
...
Это **основной ограничитель скорости**, а не запасной: идём быстро, а backoff
реагирует на реальный сигнал Steam (адаптивно к любому истинному пределу — его
точное число мы не хардкодим). Отличие от `free_games` (тот ретраит
RateLimitExceeded **бесконечно** — там единственная стена это license-cap). Для
вишлиста бесконечный ретрай **опасен**: долбёжка 429 продлевает IP soft-ban.
Поэтому:
...
| `--interval SEC` | пауза между add (дефолт ~1с; `0` = макс. скорость) |
