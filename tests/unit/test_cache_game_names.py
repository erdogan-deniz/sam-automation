...
def _patch(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        cache_mod, "GAME_NAMES_FILE", tmp_path / "game_names.json"
    )
...
def test_load_game_names_missing_file(
...
def test_load_game_names_corrupted_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _patch(monkeypatch, tmp_path)
...
def test_keys_are_integers_after_load(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _patch(monkeypatch, tmp_path)
