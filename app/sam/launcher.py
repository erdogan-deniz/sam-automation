...
def kill_all_sam_games() -> None:
    ...
    active их не тронет — они бы осиротели. run-lock гарантирует, что нет ДРУГОГО
    нашего скрипта (farm/boost), поэтому живые SAM.Game.exe — это наши сироты.
    ОГОВОРКА: вручную запущенный пользователем SAM.Game.exe run-lock НЕ исключает
    — он тоже будет убит (на автомат-машине это приемлемо).
    """
