...
class SettingsTab(ctk.CTkScrollableFrame):
    ...
    def _build_ui(self) -> None:
        ...
        self._banner.grid(
            row=row, column=0, columnspan=3, padx=8, pady=(8, 0), sticky="w"
        )
        ...
        self._sam_exe, row = self._path_field(
            "sam_game_exe_path", row, kind="file"
        )
        self._steam_path, row = self._path_field("steam_path", row, kind="dir")
        self._game_ids_file, row = self._path_field(
            "game_ids_file", row, kind="txt"
        )
        ...
        self._max_consecutive_errors = self._field(
            "max_consecutive_errors", row
        )
        row += 1
        ...
        self._playtime_idle_duration = self._field(
            "playtime_idle_duration (seconds)", row
        )
        row += 1
        ...
        self._exclude_ids = ctk.CTkTextbox(
            self, height=80, font=("Consolas", 12)
        )
        self._exclude_ids.grid(
            row=row, column=0, columnspan=3, padx=8, pady=4, sticky="ew"
        )
        row += 1
        ...
        btn.grid(
            row=row, column=0, columnspan=3, padx=8, pady=(4, 16), sticky="ew"
        )
    ...
    def _section(self, title: str, row: int) -> int:
        ...
        ctk.CTkLabel(
            ...
            row=row, column=0, columnspan=3, padx=8, pady=(12, 2), sticky="w"
        )
        return row + 1
    ...
    def _field(self, label: str, row: int, show: str = "") -> ctk.CTkEntry:
        ...
        entry.grid(
            row=row, column=1, columnspan=2, padx=(0, 8), pady=2, sticky="ew"
        )
        return entry
    ...
    def is_configured(self) -> bool:
        ...
        return bool(
            self._steam_api_key.get().strip() and self._steam_id.get().strip()
        )
